...
namespace JCAT {
    ...
    JCATDescriptorSetLayout::Builder &JCATDescriptorSetLayout::Builder::addBinding(uint32_t binding,
    ...
    JCATDescriptorSetLayout::JCATDescriptorSetLayout(DeviceSetup &device, std::unordered_map<uint32_t, VkDescriptorSetLayoutBinding> bindings)
        : device{device}, bindings{bindings} {

        std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings{};
        for (const std::pair<uint32_t, VkDescriptorSetLayoutBinding>& kv : bindings) {
    ...
    JCATDescriptorPool::Builder &JCATDescriptorPool::Builder::addPoolSize(VkDescriptorType descriptorType, uint32_t count) {
        poolSizes.push_back({descriptorType, count});
        return *this;
    }

    JCATDescriptorPool::Builder &JCATDescriptorPool::Builder::setPoolFlags(VkDescriptorPoolCreateFlags flags){
        poolFlags = flags;
        return *this;
    }
    ...
    JCATDescriptorPool::JCATDescriptorPool(DeviceSetup &device,
                                           uint32_t maxSets,
    ...
    bool JCATDescriptorPool::allocateDescriptor(const VkDescriptorSetLayout descriptorSetLayout, VkDescriptorSet &descriptor) const {
    ...
    JCATDescriptorWriter::JCATDescriptorWriter(JCATDescriptorSetLayout &setLayout, JCATDescriptorPool &pool) : setLayout{setLayout}, pool{pool} {}

    JCATDescriptorWriter &JCATDescriptorWriter::writeBuffer(uint32_t binding, VkDescriptorBufferInfo *bufferInfo) {
    ...
    JCATDescriptorWriter &JCATDescriptorWriter::writeImage(uint32_t binding, VkDescriptorImageInfo *imageInfo) {
        assert(setLayout.bindings.count(binding) == 1 && "Layout does not contain specified binding");
